...
def read_xlsx_L2_Hit_Rate(file_name="", NCU_sheet_name="", PPT_sheet_name="", ASIM_sheet_name="", OURS_sheet_name=""):
    ...
    for kernel_key in NCU_L2_Hit_Rate.keys():
        NCU_L2_Hit_Rate_Error_Rate[kernel_key] = abs(NCU_L2_Hit_Rate[kernel_key] - NCU_L2_Hit_Rate[kernel_key]) / NCU_L2_Hit_Rate[kernel_key]
        if kernel_key in PPT_L2_Hit_Rate.keys():
            PPT_L2_Hit_Rate_Error_Rate[kernel_key] = abs(PPT_L2_Hit_Rate[kernel_key] - NCU_L2_Hit_Rate[kernel_key]) / NCU_L2_Hit_Rate[kernel_key]
        if kernel_key in ASIM_L2_Hit_Rate.keys():
            ASIM_L2_Hit_Rate_Error_Rate[kernel_key] = abs(ASIM_L2_Hit_Rate[kernel_key] - NCU_L2_Hit_Rate[kernel_key]) / NCU_L2_Hit_Rate[kernel_key]
        if kernel_key in OURS_L2_Hit_Rate.keys():
            OURS_L2_Hit_Rate_Error_Rate[kernel_key] = abs(OURS_L2_Hit_Rate[kernel_key] - NCU_L2_Hit_Rate[kernel_key]) / NCU_L2_Hit_Rate[kernel_key]
    ...
    for key in NCU_L2_Hit_Rate.keys():
        ...
        print(key, ASIM_L2_Hit_Rate[key] - NCU_L2_Hit_Rate[key], PPT_L2_Hit_Rate[key] - NCU_L2_Hit_Rate[key], OURS_L2_Hit_Rate[key] - NCU_L2_Hit_Rate[key])
    ...
    return NCU_L2_Hit_Rate, PPT_L2_Hit_Rate, ASIM_L2_Hit_Rate, OURS_L2_Hit_Rate
...
def plot_figure_L2_Hit_Rate(style_label=""):
    ...
    penguin_means = {}
    penguin_means['NCU'] = tuple(float("%.2f" % float(NCU_L2_Hit_Rate_Error_Rate_list[_])) for _ in range(len(NCU_L2_Hit_Rate_Error_Rate_list)))
    penguin_means['ASIM'] = tuple(float("%.2f" % float(ASIM_L2_Hit_Rate_Error_Rate_list[_])) for _ in range(len(ASIM_L2_Hit_Rate_Error_Rate_list)))
    penguin_means['PPT'] = tuple(float("%.2f" % float(PPT_L2_Hit_Rate_Error_Rate_list[_])) for _ in range(len(PPT_L2_Hit_Rate_Error_Rate_list)))
    penguin_means['HyFiSS'] = tuple(float("%.2f" % float(OURS_L2_Hit_Rate_Error_Rate_list[_])) for _ in range(len(OURS_L2_Hit_Rate_Error_Rate_list)))

    cluster_name = ['NCU', 'ASIM', 'PPT', 'HyFiSS']

    x = np.arange(len(species))
    width = 0.16
    multiplier = 0
    color = ['#4eab90', '#edddc3', '#eebf6d', '#c55a11']
    hatch = ['-', '//', '\\\\', 'x']
    cluster_name = ['NCU', 'ASIM', 'PPT', 'HyFiSS']
    ...
    axs.set_ylabel('Simulated L2 Hit Rate', fontsize=15, y=0.5)
    axs.set_title('')

    axs.legend(loc='best', fontsize=11, frameon=True, shadow=True, fancybox=False, framealpha=1.0, borderpad=0.3,
               ncol=1, markerfirst=True, markerscale=0.2, numpoints=1, handlelength=1.2, handleheight=0.5)
    ...
    for style_label in style_list:
        ...
        with plt.rc_context({"figure.max_open_warning": len(style_list)}):
            with plt.style.context(style_label):
                ...
                plt.savefig('figs/'+'Bar_L2_Hit_Rate.pdf', format='pdf')
