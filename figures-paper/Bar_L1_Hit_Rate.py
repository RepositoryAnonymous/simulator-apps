...
def plot_bar_x_application_y_L1_Hit_Rate_Error_Rate(ax, indexes, width, cycles, bar_position=0, color="", label="", hatch=""):
    """bar plot."""
    '''
...
def read_xlsx_L1_Hit_Rate(file_name="", NCU_sheet_name="", PPT_sheet_name="", ASIM_sheet_name="", OURS_sheet_name=""):
    ...
    return NCU_L1_Hit_Rate, PPT_L1_Hit_Rate, ASIM_L1_Hit_Rate, OURS_L1_Hit_Rate
...
def plot_figure_L1_Hit_Rate(style_label=""):
    ...
    penguin_means = {}
    penguin_means['NCU'] = tuple(float("%.2f" % float(NCU_L1_Hit_Rate_Error_Rate_list[_])) for _ in range(len(NCU_L1_Hit_Rate_Error_Rate_list)))
    ...
    penguin_means['HyFiSS'] = tuple(float("%.2f" % float(OURS_L1_Hit_Rate_Error_Rate_list[_])) for _ in range(len(OURS_L1_Hit_Rate_Error_Rate_list)))
    ...
    axs.set_ylabel('Simulated L1 Hit Rate', fontsize=15, y=0.5)
    axs.set_title('')
    ...
    for style_label in style_list:
        ...
        with plt.rc_context({"figure.max_open_warning": len(style_list)}):
            with plt.style.context(style_label):
                ...
                plt.savefig('figs/'+'Bar_L1_Hit_Rate.pdf', format='pdf')
