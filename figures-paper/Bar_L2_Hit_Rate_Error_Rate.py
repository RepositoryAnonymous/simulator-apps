...
def plot_figure_L2_Hit_Rate(style_label=""):
    ...
    penguin_means = {}
    penguin_means['ASIM\ \ \ (MAE: 14.89\%, Corr.: 0.824)'] = tuple(float("%.2f" % float(ASIM_L2_Hit_Rate_Error_Rate_list[_])) for _ in range(len(ASIM_L2_Hit_Rate_Error_Rate_list)))
    penguin_means['PPT\ \ \ \ (MAE: 13.14\%, Corr.: 0.729)'] = tuple(float("%.2f" % float(PPT_L2_Hit_Rate_Error_Rate_list[_])) for _ in range(len(PPT_L2_Hit_Rate_Error_Rate_list)))
    penguin_means['HyFiSS\ (MAE: 13.53\%, Corr.: 0.720)'] = tuple(float("%.2f" % float(OURS_L2_Hit_Rate_Error_Rate_list[_])) for _ in range(len(OURS_L2_Hit_Rate_Error_Rate_list)))

    cluster_name = ['PPT\ \ \ \ (MAE: 13.14\%, Corr.: 0.729)', \
                    'ASIM\ \ \ (MAE: 14.89\%, Corr.: 0.824)', \
                    'HyFiSS\ (MAE: 13.53\%, Corr.: 0.720)']
    ...
    for style_label in style_list:
        ...
        with plt.rc_context({"figure.max_open_warning": len(style_list)}):
            with plt.style.context(style_label):
                ...
                plt.savefig('figs/'+'Bar_L2_Hit_Rate_Error_Rate.pdf', format='pdf')
