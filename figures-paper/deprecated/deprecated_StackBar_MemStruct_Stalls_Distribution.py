...
def read_xlsx_MemStruct_Stalls(file_name="", OURS_sheet_name=""):
    ...
    InterconnectInjectionBuferSaturationCycles = {}
    ...
    for idx, row in data_OURS.iterrows():
        ...
        kernel_FunctionalUnitPipelineSaturationCycles = row["MemoryStructuralStall_Issue_out_has_no_free_slot_Cycles"]
        kernel_FunctionalUnitIssuingMutualExclusionCycles = row["MemoryStructuralStall_Issue_previous_issued_inst_exec_type_is_memory_Cycles"]
        kernel_ResultBusSaturationCycles = row["MemoryStructuralStall_Execute_result_bus_has_no_slot_for_latency_Cycles"]
        kernel_DispatchQueueSaturationCycles = row["MemoryStructuralStall_Execute_m_dispatch_reg_of_fu_is_not_empty_Cycles"]
        kernel_BankConflictCycles = row["MemoryStructuralStall_Writeback_bank_of_reg_is_not_idle_Cycles"]
        kernel_BankConflict1Cycles = row["MemoryStructuralStall_ReadOperands_bank_reg_belonged_to_was_allocated_Cycles"]
        kernel_NoFreeOperandsCollectorUnitCycles = row["MemoryStructuralStall_ReadOperands_port_num_m_in_ports_m_in_fails_as_not_found_free_cu_Cycles"]
        kernel_InterconnectInjectionBuferSaturationCycles = row["MemoryStructuralStall_Execute_icnt_injection_buffer_is_full_Cycles"]
        ...
        if not kernel_key in Except_keys:
            if not kernel_key in FunctionalUnitPipelineSaturationCycles.keys():
                if isinstance(kernel_FunctionalUnitPipelineSaturationCycles, (int, float)) and \
                    ...
                    not pd.isna(kernel_NoFreeOperandsCollectorUnitCycles) and \
                    isinstance(kernel_InterconnectInjectionBuferSaturationCycles, (int, float)) and \
                    not pd.isna(kernel_InterconnectInjectionBuferSaturationCycles):
                    ...
                    InterconnectInjectionBuferSaturationCycles[kernel_key] = kernel_InterconnectInjectionBuferSaturationCycles
                else:
                    continue
            else:
                if isinstance(kernel_FunctionalUnitPipelineSaturationCycles, (int, float)) and \
    ...
    InterconnectInjectionBuferSaturationCycles_rate = {}
    ...
    for key in FunctionalUnitPipelineSaturationCycles.keys():
        ...
        print(FunctionalUnitPipelineSaturationCycles[key], FunctionalUnitIssuingMutualExclusionCycles[key], \
            ...
            NoFreeOperandsCollectorUnitCycles[key], InterconnectInjectionBuferSaturationCycles[key])
        ...
        InterconnectInjectionBuferSaturationCycles_rate[key] = \
            float(InterconnectInjectionBuferSaturationCycles[key]) / all_cycles
    ...
    return FunctionalUnitPipelineSaturationCycles_rate, FunctionalUnitIssuingMutualExclusionCycles_rate, \
           ...
           BankConflictCycles_rate, \
           NoFreeOperandsCollectorUnitCycles_rate, InterconnectInjectionBuferSaturationCycles_rate


def plot_figure_StackBar_System_Stalls(style_label=""):
    """Setup and plot the demonstration figure with a given style."""
    prng = read_xlsx_MemStruct_Stalls(
                "compare.xlsx", 
    ...
    for key in FunctionalUnitPipelineSaturationCycles_rate.keys():
        penguin_means[key] =      ( float("%.6f" % float(FunctionalUnitPipelineSaturationCycles_rate[key])), \
                                    ...
                                    float("%.6f" % float(InterconnectInjectionBuferSaturationCycles_rate[key]))
                                  )
    ...
    for i in range(len(data[0])):
        ax.bar(labels, [d[i] for d in data], width=width, bottom=bottom, color=colors[i], hatch=hatch[i], edgecolor='black')
    ...
    ax.set_ylabel('MemStruct Dist.', fontsize=14.5)
    ax.set_title('')
    ...
    ax.legend(['Functional Unit Pipeline Saturation', \
               ...
               'Interconnect Injection Bufer Saturation'], \
              ...
              loc='upper center', bbox_to_anchor=(0.5, -0.34), ncol=4)
    ...
    for style_label in style_list:
        ...
        with plt.rc_context({"figure.max_open_warning": len(style_list)}):
            with plt.style.context(style_label):
                ...
                plt.savefig('figs/'+'StackBar_MemStruct_Stalls_Distribution.pdf', format='pdf')
