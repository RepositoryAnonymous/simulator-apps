...
Except_keys = [
    ...
    "cublas_GemmEx_HF_CC_example_2048x2048x2048",
    "GRU"
]

def plot_bar_x_application_y_L2_Hit_Rate_Error_Rate(ax, indexes, width, cycles, bar_position=0, color="", label=""):
    """bar plot."""
    '''
...
def plot_figure_L2_Hit_Rate(style_label=""):
    ...
    fig, axs = plt.subplots(ncols=1, nrows=1, num=style_label,
                            figsize=(15, 5.8), layout='constrained', dpi=300)
    ...
    penguin_means['OURS'] = tuple(float("%.2f" % float(OURS_L2_Hit_Rate_Error_Rate_list[_])) for _ in range(len(OURS_L2_Hit_Rate_Error_Rate_list)))
    ...
    axs.set_ylabel('Simulated L2 Hit Rate Err.', fontsize=20)
    axs.set_title('')
    ...
    for style_label in style_list:
        ...
        with plt.rc_context({"figure.max_open_warning": len(style_list)}):
            with plt.style.context(style_label):
                ...
                plt.savefig('figs/'+style_label_name+'_GPU_L2_Hit_Rate_Error_Rate.eps', format='eps')
                plt.savefig('figs/'+style_label_name+'_GPU_L2_Hit_Rate_Error_Rate.png', format='png')
